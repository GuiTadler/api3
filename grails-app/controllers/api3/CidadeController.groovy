...
class CidadeController implements ExceptionHandlers  {
    ...
    def save() {
        ...
        if (resp.status == 200) {
            Map dadosLog = [operation: 'POST', situation: 'Success', resource: 'Cidade', resourceId: resp.json.id.toString()]
    ...
    def update(Long id) {
        ...
        if (resp.status == 200) {
            Map dadosLog = [operation: 'PUT', situation: 'Success', resource: 'Cidade', resourceId: id.toString()]
            logService.salvarLog(dadosLog)
        }
        respond(resp.json, status: resp.status)
    }
    ...
    def delete(Long id) {
        ...
        if (resp.status == 200) {
            Map dadosLog = [operation: 'DELETE', situation: 'Success', resource: 'Cidade', resourceId: id.toString()]
            logService.salvarLog(dadosLog)
        }
        respond(resp.json, status: resp.status)
    }
